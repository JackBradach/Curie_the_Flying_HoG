...
/* Used by hog_notify() to hit the right descriptor. */
#define ATTR_INPUT_REPORT_OFFSET 6
...
uint8_t mouse_status[3] = {0, 0, 0};
void hog_notify(int dx, int dy, uint8_t btn_msk)
{
	mouse_status[0] = btn_msk;
	mouse_status[1] = dx;
	mouse_status[2] = dy;
	
	bt_gatt_notify(NULL, &attrs[ATTR_INPUT_REPORT_OFFSET], &mouse_status, sizeof(mouse_status));
}